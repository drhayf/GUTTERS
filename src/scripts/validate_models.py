...
def validate_models() -> tuple[int, list[str], list[str]]:
    ...
    for model_file in model_files:
        ...
        try:
            ...
            # Get the model class (usually PascalCase of filename)
            class_name = ''.join(word.capitalize() for word in model_name.split('_'))
            if hasattr(module, class_name):
                model_class = getattr(module, class_name)
                # Verify it has __tablename__
                if not hasattr(model_class, '__tablename__'):
                    errors.append(f"❌ {model_name}: Missing __tablename__")
                    continue
...
def main():
    ...
    # Summary
    total_passed = len(passed_models) + len(passed_protocol) + len(passed_core) + len(passed_schemas) + len(passed_utils)
