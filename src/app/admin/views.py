...
class UserProfileCreate(BaseModel):
    """Schema for creating UserProfile from admin."""
    user_id: int
    data: dict = Field(default_factory=dict)


class UserProfileUpdate(BaseModel):
    """Schema for updating UserProfile from admin."""
    data: dict | None = None
...
def register_admin_views(admin: CRUDAdmin) -> None:
    ...
    # UserProfile - view cosmic profile data for debugging
    admin.add_view(
        model=UserProfile,
        create_schema=UserProfileCreate,
        update_schema=UserProfileUpdate,
        allowed_actions={"view", "update"},  # Read-only + update for fixes
    )
