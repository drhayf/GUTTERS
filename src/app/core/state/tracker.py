...
Example:
    >>> from app.core.state.tracker import get_state_tracker
...
class StateTracker:
        ...
        async for db in async_get_db():
            ...
            if profile is None:
                return {
                    ...
                    "domains": {name: {"complete": False, "confidence": 0.0} 
                              for name in PROFILE_DOMAINS},
                    "last_synthesis": None,
                    "total_data_points": 0,
                }
            ...
            last_synthesis = profile.get("updated_at") if isinstance(profile, dict) else getattr(profile, "updated_at", None)
    ...
    async def update_genesis_status(
        self,
        user_id: int,
    ...
    async def update_field_confirmed(
        self,
        user_id: int,
        field: str,
        confirmed_value: str
