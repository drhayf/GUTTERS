                ...
                try:
                    ...
                    if p_value < self.significance_threshold and t_stat > 0:
                        ...
                        patterns.append(CyclicalPattern(
                            ...
                            finding=(
                                f"User reports {symptom} {avg_rate*100:.0f}% of the time "
                                f"during {planet.value} periods (baseline: {baseline_rate*100:.0f}%, "
                                f"{fold_increase:.1f}x increase)"
                            )
                        ))
    ...
    def _test_metric_variance(
        ...
        try:
            ...
            return CyclicalPattern(
                ...
                finding=(
                    f"{metric_label} significantly higher during {highest_planet.value} "
                    f"periods (avg {highest_value:.1f}) vs {lowest_planet.value} periods "
                    f"(avg {lowest_value:.1f})"
                )
            )
        ...
        for planet, period_data in periods_by_planet.items():
            ...
            if avg_alignment > baseline * 1.5:
                ...
                patterns.append(CyclicalPattern(
                    ...
                    finding=(
                        f"Journal content during {planet.value} periods aligns with "
                        f"themes '{', '.join(theme_keywords[:3])}' (alignment: "
                        f"{avg_alignment:.0%} vs baseline {baseline:.0%})"
                    )
                ))
            ...
            try:
                ...
                if p_value < self.significance_threshold and abs(slope) > 0.3:
                    ...
                    patterns.append(CyclicalPattern(
                        ...
                        finding=(
                            f"Mood during {planet.value} periods {trend_direction} "
                            f"across {len(years)} years (slope: {slope:.2f}, "
                            f"R²: {r_squared:.2f})"
                        )
                    ))
