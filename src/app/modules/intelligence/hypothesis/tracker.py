...
from .models import Hypothesis, HypothesisUpdate, HypothesisStatus, HypothesisType
...
class HypothesisTracker:
    ...
    async def update_from_new_data(
        ...
        if evidence_date_str:
            try:
                ...
                if dt.datetime.now(dt.timezone.utc) - evidence_date > dt.timedelta(days=60):
        ...
        hypothesis.last_updated = dt.datetime.now(dt.timezone.utc)
        hypothesis.last_evidence_at = dt.datetime.now(dt.timezone.utc)
    ...
    async def _evaluate_evidence(
        ...
        if hypothesis.hypothesis_type == HypothesisType.COSMIC_SENSITIVITY and "solar" in hypothesis.predicted_value.lower():
        ...
        # Lunar pattern hypothesis
        elif hypothesis.hypothesis_type == HypothesisType.COSMIC_SENSITIVITY and "moon" in hypothesis.predicted_value.lower():
            if data_type == "journal_entry":
                ...
                predicted_phase_match = any(word.lower() in lunar_phase.lower() for word in hypothesis.predicted_value.split())
        ...
        # Temporal pattern hypothesis
            ...
            if data_type == "journal_entry":
                ...
                if not entry_day and 'timestamp' in new_data:
                    try:
                        dt = dt.datetime.fromisoformat(new_data['timestamp'].replace('Z', '+00:00'))
                        entry_day = dt.strftime('%A')
                    except ValueError:
                        pass
    ...
    async def check_stale_hypotheses(
        ...
        stale_threshold = dt.datetime.now(dt.timezone.utc) - dt.timedelta(days=60)
