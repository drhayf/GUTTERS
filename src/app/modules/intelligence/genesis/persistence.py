...
class GenesisPersistence:
    ...
    async def cache_declaration(self, declaration: UncertaintyDeclaration) -> bool:
        """
        ...
        try:
            ...
            await self.redis.setex(key, self.CACHE_TTL, declaration.model_dump_json())
            return True
        except Exception:
            return False

    async def get_cached_declaration(self, user_id: int, session_id: str) -> UncertaintyDeclaration | None:
        """
    ...
    async def store_to_profile(self, declaration: UncertaintyDeclaration, db_session: Any) -> bool:
        """
        ...
        try:
            ...
            # Get user profile
            result = await db_session.execute(select(UserProfile).where(UserProfile.user_id == declaration.user_id))
            ...
            profile.data["genesis"]["uncertainties"][declaration.module] = declaration.to_storage_dict()
    ...
    async def get_from_profile(self, user_id: int, module_name: str, db_session: Any) -> UncertaintyDeclaration | None:
        """
    ...
    async def get_all_from_profile(self, user_id: int, db_session: Any) -> list[UncertaintyDeclaration]:
        """
    ...
    async def store(self, declaration: UncertaintyDeclaration, db_session: Any | None = None) -> bool:
        """
    ...
    async def get(
        self, user_id: int, session_id: str, module_name: str | None = None, db_session: Any | None = None
