...
from ..schemas import (
    HumanDesignChart, HDGate, HDChannel,
    TypeProbability, ChartSubject
)
from .ephemeris import get_ephemeris, EphemerisCalculator
from .mechanics import (
    get_channel_builder, get_type_determinator,
    get_authority_determinator, get_definition_calculator,
    get_profile_calculator, get_cross_calculator,
    ChannelBuilder, TypeDeterminator
)
...
class HumanDesignCalculator:
    ...
    def _calculate_full_chart(
        ...
        profile_result = self.profile_calculator.calculate_profile(
            personality_sun["line"] if personality_sun else 1,
            design_sun["line"] if design_sun else 1
        )
    ...
    def _calculate_probabilistic_type(
        self,
        birth_date: date,
        latitude: float,
        longitude: float,
        ...
        for type_name, count in type_counts.most_common():
            ...
            probabilities.append(TypeProbability(
                value=type_name,
    ...
    def calculate_natal_chart(
        ...
        if birth_time is not None:
            ...
            return {
                ...
                "channels": [
                    {"gates": ch.gates, "name": ch.name, "defined": ch.defined}
            ...
            prob_result = self._calculate_probabilistic_type(
                birth_date, latitude, longitude, timezone
            )
            
            # Get a sample chart for noon to show other properties
            sample_chart = self._calculate_full_chart(
                name, birth_date, time(12, 0),
                latitude, longitude, timezone
            )
    ...
    def calculate_chart(
        ...
        if birth_time is not None:
            ...
            return self._calculate_full_chart(
                name, birth_date, birth_time,
                latitude, longitude, timezone
            )
        else:
            # Probabilistic calculation - unknown birth time
            return self._calculate_probabilistic_chart(
                name, birth_date,
                latitude, longitude, timezone
            )
    ...
    def _calculate_probabilistic_chart(
        ...
        noon_chart = self._calculate_full_chart(
            name, birth_date, time(12, 0),
            latitude, longitude, timezone
        )
